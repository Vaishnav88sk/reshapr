#!/usr/bin/env bash
# Profiled variant of the E2E gRPC benchmark: runs a single payload campaign with a
...
#
# Usage: ./run-e2e-grpc-profile.sh [payload]      (default: large)
# Tunables: same env vars as run-e2e-grpc-bench.sh (LATENCY_RATE, THROUGHPUT_VUS, ...).
#
# Output: results-e2e-grpc/proxy-<payload>.jfr plus the usual k6/memory files.
...
cd "$(dirname "$0")"
PAYLOAD="${1:-large}"
RESULTS_DIR="$(pwd)/results-e2e-grpc"
