#!/usr/bin/env bash
#
# End-to-end benchmark of the reshapr proxy (gRPC backend), measuring — per response payload
...
#
# It is the gRPC counterpart of run-e2e-rest-bench.sh: same phases, same metrics and same summary
# tool (summarize-e2e.sh), so the three protocols (REST / GraphQL / gRPC) are comparable.
#
# Topology (everything on loopback):
#
#   k6 (injector) --> proxy (real Quarkus fast-jar, :7777) --gRPC--> MinimalGrpcBackend x3 (ephemeral)
#                        |
#                        +--gRPC--> E2EGrpcBenchEnvironment stub control-plane (EDS + GHS, :15555)
#
# Each exposition serves a synthetic Protobuf descriptor and the 'Method0' unary operation as the
# benchmarked tool (see e2e/k6/grpc-tools-call.js). The MinimalGrpcBackend returns a canned
# Protobuf response the proxy re-encodes as JSON, so the response magnitude drives the measurement.
#
...
#
# Usage: ./run-e2e-grpc-bench.sh [payloads...]      (default: small medium large)
...
#                 PAYLOAD_SMALL_BYTES/PAYLOAD_MEDIUM_BYTES/PAYLOAD_LARGE_BYTES
#                 (approx. Protobuf response bytes; defaults 5120/51200/524288
...
PAYLOAD_SMALL_BYTES="${PAYLOAD_SMALL_BYTES:-5120}"
PAYLOAD_MEDIUM_BYTES="${PAYLOAD_MEDIUM_BYTES:-51200}"
PAYLOAD_LARGE_BYTES="${PAYLOAD_LARGE_BYTES:-524288}"
AUDIT_ENABLED="${AUDIT_ENABLED:-false}"

RESULTS_DIR="$BENCH_DIR/results-e2e-grpc"
mkdir -p "$RESULTS_DIR"
...
# --- 1. Start the E2E environment (stub control-plane + canned gRPC backends) -----------------
echo "Starting E2E environment (stub control-plane :$GRPC_PORT + 3 minimal gRPC backends)..."
"$JAVA" --enable-preview -De2e.grpc.port="$GRPC_PORT" \
  -De2e.payload.small.bytes="$PAYLOAD_SMALL_BYTES" \
  -De2e.payload.medium.bytes="$PAYLOAD_MEDIUM_BYTES" \
  -De2e.payload.large.bytes="$PAYLOAD_LARGE_BYTES" \
  -De2e.audit.enabled="$AUDIT_ENABLED" \
  -cp "$BENCH_DIR/target/benchmarks.jar" io.reshapr.benchmarks.e2e.E2EGrpcBenchEnvironment \
...
run_k6() {
  ...
  if [ "$K6_MODE" = "native" ]; then
    ...
    k6 run --quiet ${extra[@]+"${extra[@]}"} \
      ...
      "$BENCH_DIR/e2e/k6/grpc-tools-call.js"
  else
