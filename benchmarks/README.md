...
## End-to-end benchmarks (`run-e2e-rest-bench.sh`, `run-e2e-graphql-bench.sh`, `run-e2e-grpc-bench.sh`)

The JMH benchmarks above isolate single components in-JVM. The **end-to-end benchmarks** measure
...
- `io.reshapr.benchmarks.e2e.E2EBenchEnvironment` hosts a **stub control-plane** (plain gRPC
  server implementing `eds-v1.proto` + `ghs-v1.proto`, factored out into the shared
  `E2EStubControlPlane`) serving three REST expositions (`bench-rest-small|medium|large`) whose
  main artifact is a synthetic OpenAPI spec generated by `OpenAPISpecGenerator`, plus three
  **canned backends** (`CannedHttpBackend`, the fixed-port variant of `MinimalHttpBackend`)
  answering with ~5 KiB / ~50 KiB / ~500 KiB JSON bodies (configurable item counts, see below) —
  fast enough to never be the bottleneck.
...
### GraphQL and gRPC backends

The GraphQL and gRPC end-to-end benchmarks reuse the exact same infrastructure, phases and metrics
as the REST one — only the exposition type, the artifact and the backend differ — so results are
directly comparable, and both are (re-)summarized with the same `summarize-e2e.sh`.

```bash
./run-e2e-graphql-bench.sh              # GraphQL, all three payload sizes -> results-e2e-graphql/
./run-e2e-grpc-bench.sh                 # gRPC,    all three payload sizes -> results-e2e-grpc/
./summarize-e2e.sh results-e2e-graphql  # same 3 tables as REST
./summarize-e2e.sh results-e2e-grpc
```

- **GraphQL** (`E2EGraphQLBenchEnvironment`, `e2e/k6/graphql-tools-call.js`): three
  `bench-graphql-small|medium|large` expositions whose main artifact is the real-world GitHub
  GraphQL schema (`github-api.graphql`, ~1.4 MiB, bundled in `src/main/resources` — same schema as
  the GraphQL micro-benchmark). k6 drives the `user` query tool. Since GraphQL-over-HTTP is a plain
  `POST` whose JSON response the proxy returns verbatim, the schema-agnostic `CannedHttpBackend` is
  reused unchanged, answering with ~5 KiB / ~50 KiB / ~500 KiB bodies
  (`PAYLOAD_SMALL_ITEMS|MEDIUM_ITEMS|LARGE_ITEMS`, same defaults 38/380/3700 as REST).
- **gRPC** (`E2EGrpcBenchEnvironment`, `e2e/k6/grpc-tools-call.js`): three
  `bench-grpc-small|medium|large` expositions whose main artifact is a synthetic base64
  `FileDescriptorSet` produced by `GrpcSpecGenerator` (`PROTOBUF_DESCRIPTOR`). k6 drives the
  `Method0` unary tool. Each backend is a `MinimalGrpcBackend` (reused from the gRPC
  micro-benchmark) returning a canned `ResponseMessage0` padded to ~5 KiB / ~50 KiB / ~500 KiB
  (`PAYLOAD_SMALL_BYTES|MEDIUM_BYTES|LARGE_BYTES`, defaults 5120/51200/524288); the proxy
  re-encodes it as JSON so the response magnitude drives the measurement.

All the shared tunables (`LATENCY_RATE`, `THROUGHPUT_VUS`, `PROXY_HEAP`, `AUDIT_ENABLED`, payload
subset arguments, …) behave identically across the three scripts.
...
`run-e2e-rest-bench.sh`, `run-e2e-graphql-bench.sh` and `run-e2e-grpc-bench.sh` run the
end-to-end campaigns (see above).
