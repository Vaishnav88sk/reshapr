#!/usr/bin/env bash
#
# End-to-end benchmark of the reshapr proxy (REST backend), measuring — per response payload
...
#
#   k6 (injector) --> proxy (real Quarkus fast-jar, :7777) --> CannedHttpBackend x3 (:19901-19903)
#                        |
#                        +--gRPC--> E2EBenchEnvironment stub control-plane (EDS + GHS, :15555)
#
# Per payload size, three k6 phases are run (see e2e/k6/rest-tools-call.js):
...
#
# Usage: ./run-e2e-rest-bench.sh [payloads...]      (default: small medium large)
...
#                 AUDIT_ENABLED (default false: set to true to enable the audit flag on every
#                  exposition configuration and measure the audit-path overhead. Note: the OTEL
#                  SDK stays disabled, so the final log emission is a no-op — what is measured
#                  is the audit event construction path, including response re-serialization).
set -euo pipefail
...
RESULTS_DIR="$BENCH_DIR/results-e2e-rest"
mkdir -p "$RESULTS_DIR"
...
# --- 1. Start the E2E environment (stub control-plane + canned backends) ---------------------
echo "Starting E2E environment (stub control-plane :$GRPC_PORT + 3 canned backends)..."
"$JAVA" --enable-preview -De2e.grpc.port="$GRPC_PORT" \
  ...
  -cp "$BENCH_DIR/target/benchmarks.jar" io.reshapr.benchmarks.e2e.E2EBenchEnvironment \
...
run_k6() {
  ...
  if [ "$K6_MODE" = "native" ]; then
    ...
    k6 run --quiet ${extra[@]+"${extra[@]}"} \
      ...
      "$BENCH_DIR/e2e/k6/rest-tools-call.js"
  else
