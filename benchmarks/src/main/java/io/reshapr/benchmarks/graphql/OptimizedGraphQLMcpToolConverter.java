...
/**
 * Optimized variant of {@code GraphQLMcpToolConverter}. Two changes, everything else identical:
 *
 * <ol>
 *   <li><b>Type definitions index</b> ({@link #getDocumentIndex(Document)}): a {@code DocumentIndex}
 *   (operation/object/enum/input definitions by name + custom scalar names) is built once per artifact
 *   and cached in the shared {@link WorkCache} next to the parsed {@link Document}. The lookup methods
 *   ({@code getOperationDefinition}, {@code getTypeDefinition}, {@code getEnumTypeDefinition},
 *   {@code getInputValueDefinition}, {@code isScalarType}) become O(1) map lookups instead of linear
 *   scans of all document definitions on every call.</li>
 *
 *   <li><b>Cached field-selection skeletons</b> ({@link #getOutputFieldsSkeleton},
 *   {@link #getRelationFieldsSkeleton}): the request-independent parts of the generated query (the
 *   recursive selection of all argument-less fields of the operation output type, and of each relation
 *   type) are built once and cached as plain strings in the {@link WorkCache}. Only the (small)
 *   request-dependent argument lists are built per call.</li>
 * </ol>
 *
 * @author laurent
 */
public class OptimizedGraphQLMcpToolConverter extends McpToolConverter {
   ...
   /**
    * OPTIMIZATION #1: name-indexed views over the parsed document, built once and cached in the
   ...
   public OptimizedGraphQLMcpToolConverter(ExpositionEntry exposition, WorkCache workCache,
   ...
   /** OPTIMIZATION #1: get (or build once) the name-indexed views over the document. */
   private DocumentIndex getDocumentIndex(Document document) {
      String major = artifact.id();
      String minor = CACHE_KEYS_PREFIX + "index";
      Object value = workCache.get(major, minor);
      if (value instanceof DocumentIndex index) {
         logger.debugf("Got a cached value of DocumentIndex for service '%s'", service.id());
         return index;
      }
      ...
      DocumentIndex index = new DocumentIndex(operationDefinitions, objectTypes, enumTypes, inputTypes, scalarNames);
      workCache.set(major, minor, index);
      return index;
   }
   ...
   private FieldDefinition getOperationDefinition(Document document, String operationName) {
      // OPTIMIZATION #1: O(1) lookup in the cached index instead of scanning all type definitions.
   ...
   private boolean isScalarType(Document graphqlDocument, String typeName) {
      // OPTIMIZATION #1: O(1) lookup in the cached index instead of scanning scalar definitions.
   ...
   private EnumTypeDefinition getEnumTypeDefinition(Document graphqlDocument, String typeName) {
      // OPTIMIZATION #1: O(1) lookup in the cached index instead of scanning all enum definitions.
   ...
   private InputObjectTypeDefinition getInputValueDefinition(Document graphqlDocument, String typeName) {
      // OPTIMIZATION #1: O(1) lookup in the cached index instead of scanning all input definitions.
   ...
   /**
    * OPTIMIZATION #2: request-independent selection of all argument-less fields of the operation
   ...
   private String getOutputFieldsSkeleton(Document graphqlDocument, FieldDefinition operationDefinition) {
      ...
      Object value = workCache.get(major, minor);
      if (value instanceof String skeleton) {
         logger.debugf("Got a cached value of output fields skeleton for operation '%s'", operationDefinition.getName());
         return skeleton;
      }
      ...
      String skeleton = builder.toString();
      workCache.set(major, minor, skeleton);
      return skeleton;
   }

   /**
    * OPTIMIZATION #2: request-independent selection of the argument-less fields of a relation type,
   ...
   private String getRelationFieldsSkeleton(Document graphqlDocument, ObjectTypeDefinition relationTypeDef) {
      ...
      Object value = workCache.get(major, minor);
      if (value instanceof String skeleton) {
         logger.debugf("Got a cached value of relation fields skeleton for type '%s'", relationTypeDef.getName());
         return skeleton;
      }
   ...
   private void addOutputFieldsToFetch(Document graphqlDocument, FieldDefinition operationDefinition,
      ...
      // OPTIMIZATION #2: the argument-less fields selection is request-independent and cached.
         ...
         if (requestField.startsWith(RELATION_PREFIX)) {
            ...
            if (relationFieldDef != null) {
               ...
               if (relationTypeDef != null) {
                  // OPTIMIZATION #2: the relation fields selection is request-independent and cached.
